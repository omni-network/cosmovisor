...
var newConfig = func(
	...
	return &Config{
		...
		CustomCurrentLink:        customCurrentLink,
...
func (s *argsTestSuite) TestGetConfigFromEnv() {
	...
	tests := []struct {
		...
		{
			name:             "all good",
			envVals:          cosmovisorEnv{absPath, "testname", "true", "true", "false", "600ms", "true", "", "303ms", "1", "false", "true", "kitchen", "preupgrade.sh", "current", filepath.Join(absPath, "cosmovisor"), "true", "10s"},
			expectedCfg:      newConfig(absPath, "testname", true, true, false, 600, true, absPath, 303, 1, false, true, time.Kitchen, "preupgrade.sh", "current", filepath.Join(absPath, "cosmovisor"), true, 10000000000),
			expectedErrCount: 0,
		},
		{
			name:             "nothing set",
